...
class InterviewAgent(Agent):
    ...
    async def on_enter(self):
        ...
        try:
            ...
            if self.interview_language == "arabic":
                greeting_instruction = f"احكي مرحبا وعرف نفسك كرامي من {self.company_name} وابدأ المقابلة مع {self.candidate_name} لوظيفة {self.job_name}. احكي بالعربي الفصحى."
            else:
    ...
    def _build_instructions(self) -> str:
        ...
        if self.interview_language == "arabic":
            base = (
                f"أنت رامي، مسؤول موارد بشرية من {self.company_name}. "
                f"تقوم بإجراء مقابلة عمل لمنصب ({self.job_name}) مع {self.candidate_name}. "
                f"مهم جداً: يجب أن تتحدث بالعربية الفصحى فقط في جميع الأوقات، مع نطق الحركات بشكل صحيح "
                f"واستخدام النبرة المناسبة في نهاية الجمل والأسئلة. "
                f"شخصيتك: مدير موارد بشرية يتمتع بخبرة واسعة في التوظيف والاختيار، "
                f"التزم بنحوٍ وصرفٍ صحيحين دائماً: راعِ الإعراب في أواخر الكلمات، طابق الجنس والعدد، "
                f"وحافظ على اتساق الضمائر، وصحّح نفسك بصمت قبل الإرسال وأعد صياغة أي جملة مُلتبسة. "
                f"يمتاز بالهدوء والإنصات الجيد، ويطرح أسئلة واضحة وعميقة تكشف عن مهارات المرشح، "
                f"وتحافظ على جو من الاحترام والثقة طوال المقابلة. "
                f"لا تقلّد أسلوب المرشح في الكلام، بل حافظ دائماً على أسلوبك المهني الخاص."
            )
        else:
            base = (
                f"You are Rami, a professional HR interviewer from {self.company_name}. "
        ...
        if self.interview_prompt and self.interview_prompt.strip():
            ...
            if self.interview_language == "arabic":
                interview_instructions = (
                    ...
                    f"\n\nتحليل السيرة الذاتية:\n{self.cv_analysis}"
                    f"أنت المُحاور المحترف، وتدير الحوار بأسلوب رسمي ومنظم. "
                    f"مهم جداً: يجب أن تتحدث بالعربية الفصحى فقط في جميع الأوقات. لا تستخدم الإنجليزية أبداً مهما حدث.\n\n"
                    f"شخصيتك: مدير موارد بشرية يتمتع بخبرة واسعة في التوظيف والاختيار، "
                    f"يمتاز بالهدوء والإنصات الجيد، ويطرح أسئلة واضحة وعميقة تكشف عن مهارات المرشح، "
                    f"وتحافظ على جو من الاحترام والثقة طوال المقابلة. "
                    f"لا تقلّد أسلوب المرشح في الكلام، بل حافظ دائماً على أسلوبك المهني الخاص."
