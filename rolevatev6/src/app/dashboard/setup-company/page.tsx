...
export default function SetupCompanyPage() {
  ...
  const handleCreateCompany = async (e: React.FormEvent) => {
    ...
    try {
      const response = await configurationService.createCompany(companyData);
      console.log('Company created successfully:', response);
      
      // Update user in localStorage to include the new company
      if (typeof window !== 'undefined') {
        ...
        if (storedUser) {
          ...
          user.companyId = response.id;
          localStorage.setItem('user', JSON.stringify(user));
        }
      }
      
      // Reload to refresh AuthProvider with new company data
  ...
  const handleJoinCompany = async (e: React.FormEvent) => {
    ...
    console.log('Joining company with code:', invitationCode);
    
    try {
      const response = await configurationService.joinCompany({ invitationCode });
      console.log('Joined company successfully:', response);
      
      // Update user in localStorage to include the company
      if (typeof window !== 'undefined') {
        const storedUser = localStorage.getItem('user');
        if (storedUser) {
          const user = JSON.parse(storedUser);
          if (response.company) {
            ...
            user.companyId = response.company.id;
          }
          localStorage.setItem('user', JSON.stringify(user));
        }
      }
