  ...
  const signUpAndJoin = async () => {
    ...
    try {
      ...
      const signupResponse = await fetch(
        `${API_CONFIG.API_BASE_URL}/auth/signup`,
        {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      if (signupResponse.ok) {
        ...
        const joinResponse = await fetch(
          `${API_CONFIG.API_BASE_URL}/company/join`,
          {
            method: "POST",
            credentials: "include",
            headers: {
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-sm shadow-2xl p-8 max-w-md w-full border border-white/20">
        <div className="text-center mb-8">
          ...
          {invitationCode ? (
            ...
            <p className="text-gray-600">No invitation code provided</p>
          )}
        </div>
          ...
          <div className="mt-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl">
            ...
            <div className="text-sm text-green-700 space-y-1">
              ...
              {companyInfo.headquarters && (
                <p>
                  <strong>Location:</strong> {companyInfo.headquarters}
                </p>
              )}
              {companyInfo.description && (
                <p>
                  <strong>About:</strong> {companyInfo.description}
                </p>
              )}
            </div>
          </div>
