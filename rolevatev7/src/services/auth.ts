...
class AuthService {
  ...
  private parseAuthError(error: any, operation: 'login' | 'signup'): string {
    // Check for GraphQL errors with validation messages (both graphQLErrors and errors array)
    const graphQLErrors = error?.graphQLErrors || error?.errors;
    
    if (graphQLErrors?.length > 0) {
      const graphQLError = graphQLErrors[0];
      
      // Extract validation messages from originalError
      if (graphQLError?.extensions?.originalError?.message) {
        const validationMessages = graphQLError.extensions.originalError.message;
      ...
      // Fall back to the main GraphQL error message
      if (graphQLError?.message && graphQLError.message !== 'Bad Request Exception') {
    ...
    if (message && message !== 'Bad Request Exception') {
      return message;
    }
    ...
    if (operation === 'login') {
      ...
      return 'Unable to create account. This could be due to:\n• Internet connection issues\n• Invalid email format\n• Password requirements not met\n• Email already registered\n\nPlease check your information and try again.';
    }
  }
}
