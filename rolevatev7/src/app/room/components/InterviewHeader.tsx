...
export function InterviewHeader({
  ...
  return (
    <div className="bg-white/95 backdrop-blur-xl rounded-t-xl lg:rounded-t-3xl border-b border-slate-200/40 shadow-lg">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 lg:gap-0">
          ...
          <div className="flex items-center gap-2 lg:gap-6">
            <div className="relative">
              <div className="w-10 h-10 lg:w-16 lg:h-16 bg-[#0891b2] rounded-lg lg:rounded-sm flex items-center justify-center shadow-2xl animate-pulse">
              ...
              <div className="absolute inset-0 rounded-lg lg:rounded-sm border-2 border-[#13ead9]/30 animate-pulse"></div>
            </div>
              ...
              <div className="hidden sm:flex flex-wrap items-center gap-2 lg:gap-4 text-xs lg:text-sm text-slate-600">
                ...
                {companyInfo?.name && (
                  <>
                    ...
                    <span className="text-slate-600 font-medium truncate">{companyInfo.name}</span>
                  </>
                )}
                ...
                {jobInfo?.location && (
                  <>
                    <div className="hidden lg:block h-3 w-px bg-slate-300"></div>
                    <span className="text-slate-500 truncate">{jobInfo.location}</span>
                  </>
                )}
                ...
                {participantName && (
                  <>
                    <div className="hidden lg:block h-3 w-px bg-slate-300"></div>
                    <span className="text-slate-600 truncate">Candidate: {participantName}</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-1.5 lg:gap-4 overflow-x-auto">
            ...
            <div className="flex items-center gap-1.5 lg:gap-3 px-2 lg:px-5 py-1.5 lg:py-3 bg-white/90 backdrop-blur-sm rounded-lg lg:rounded-sm border border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 shrink-0">
              <div className="relative">
                <div className={`w-2.5 h-2.5 lg:w-4 lg:h-4 rounded-full ${currentStatus.bgColor} shadow-lg`}></div>
                <div className={`absolute inset-0 w-2.5 h-2.5 lg:w-4 lg:h-4 rounded-full ${currentStatus.bgColor} animate-ping opacity-75`}></div>
              </div>
              <div className="text-xs lg:text-sm">
                <div className="font-semibold text-slate-700">{currentStatus.text}</div>
                <div className="text-xs text-slate-500 hidden lg:block">AI Interviewer</div>
              </div>
            </div>
            
            {/* Session Timer - Compact on mobile */}
            <div className="flex items-center gap-1.5 lg:gap-3 px-2 lg:px-4 py-1.5 lg:py-3 bg-slate-50/90 backdrop-blur-sm rounded-lg lg:rounded-sm border border-slate-200/50 shadow-lg shrink-0">
              <ClockIcon className="w-3.5 h-3.5 lg:w-5 lg:h-5 text-slate-500" />
              <div className="text-xs lg:text-sm">
                <div className="font-mono font-semibold text-slate-700">
                  {Math.floor(sessionDuration / 60)}:{(sessionDuration % 60).toString().padStart(2, "0")}
                </div>
                <div className="text-xs text-slate-500 hidden lg:block">Duration</div>
              </div>
            </div>

            {/* Interview Progress - Hidden on mobile and tablet */}
            {jobInfo?.title && (
              <div className="hidden xl:flex items-center gap-3 px-4 py-3 bg-blue-50 rounded-sm border border-blue-200/50 shadow-lg shrink-0">
