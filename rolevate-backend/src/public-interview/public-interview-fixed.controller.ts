...
export class PublicInterviewController {
  ...
  async joinInterview(
    ...
    try {
      // Generate unique room name for this candidate using timestamp
      const timestamp = Date.now();
      const phoneDigits = phoneNumber.replace(/\D/g, '').slice(-4);
      const candidateRoomName = `${interviewSession.roomName}_${phoneDigits}_${timestamp}`;
      ...
      const candidateInterview = await this.prisma.interview.create({
        data: {
          ...
          roomCode: `${roomCode}_${phoneDigits}_${timestamp}`, // Unique code per candidate
