...
def initialize_conversation_node(state: JobPostAgentState) -> JobPostAgentState:
    ...
    print("🔧 DEBUG: Entered initialize_conversation_node")
    ...
    print(f"🔧 DEBUG: User input extracted: '{user_input}'")
...
Let's build something great together! 🚀"""
    ...
    print("🔧 DEBUG: Initialized job data structure")
...
def conversational_node(state: JobPostAgentState) -> JobPostAgentState:
    ...
    print("🔧 DEBUG: Entered conversational_node")
    ...
    try:
        ...
        if not user_message:
            print("❌ DEBUG: No user message found in state")
            return create_error_response(state, "No user message found")

        print(f"🔧 DEBUG: Processing user message: '{user_message[:100]}...'")
...
Based on their response: "{user_message}", continue the conversation to gather more job post details."""
        ...
        print("🔧 DEBUG: Extracting job info from user message...")
        updated_job_data = extract_job_info(user_message, job_data)
        print(f"🔧 DEBUG: Updated job data: {updated_job_data}")
        ...
        print(f"❌ DEBUG: Error in conversational_node: {str(e)}")
        import traceback
...
def create_job_post_graph() -> StateGraph:
    ...
    # Set up the edges - simpler flow to avoid recursion
    workflow.add_edge(START, "initialize")
    workflow.add_edge("initialize", END)  # Initialize and return directly
...
def run_job_post_agent(user_input: str, company_id: str = None, company_name: str = None) -> List[BaseMessage]:
    ...
    print(f"🔧 DEBUG: run_job_post_agent called with input: '{user_input[:50]}...'")
    print(f"🏢 DEBUG: Company: {company_name} (ID: {company_id})")
    ...
    print("🔧 DEBUG: Initial state created, invoking workflow graph...")
    
    # Run the graph
    result = job_post_graph.invoke(initial_state)
    
    print(f"🔧 DEBUG: Workflow completed, returned {len(result.get('messages', []))} messages")
    print(f"🔧 DEBUG: Final step: {result.get('current_step', 'unknown')}")
    print(f"🔧 DEBUG: Is complete: {result.get('is_complete', False)}")
