...
def send_job_post_to_api(
    ...
    try:
        ...
        # First validate and clean the job data to ensure it matches schema requirements
        validate_result = validate_job_post_data(job_data)
        ...
        # Make sure experienceLevel is properly formatted
        ...
        if experience_level.upper() not in valid_experience_levels:
            # Map to a valid value
            experience_level = map_experience_level(experience_level)
        else:
            experience_level = experience_level.upper()
        
        # Ensure salary values are integers as required
        salary_min = None
        if job_data.get("salaryMin") is not None:
            try:
                salary_min = int(float(job_data["salaryMin"]))
            except (ValueError, TypeError):
                salary_min = 0
                
        salary_max = None
        if job_data.get("salaryMax") is not None:
            try:
                salary_max = int(float(job_data["salaryMax"]))
            except (ValueError, TypeError):
                salary_max = 0
        ...
        # Ensure department field is not included
        ...
        if response.status_code in [200, 201]:
            ...
            try:
                ...
                # Check for NestJS validation errors which come in a specific format
                if response.status_code == 400 and isinstance(error_data, dict):
                    if "message" in error_data and isinstance(error_data["message"], list):
                        # Extract the detailed validation error messages
                        validation_errors = error_data["message"]
                        return {
                            "success": False,
                            "status_code": response.status_code,
                            "error": "Validation error",
                            "validation_errors": validation_errors,
                            "details": "The job post data failed validation"
                        }
...
def validate_job_post_data(job_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not api_base_url.startswith("http"):
        warnings.append("API URL may be incorrectly configured")
    
    # Check if department field exists (it shouldn't)
    if "department" in job_data:
        errors.append("property department should not exist")
        # Remove it from the job_data to avoid sending it to the API
    ...
    experience_level = job_data.get("experienceLevel", "").upper()
    valid_experience_levels = ["ENTRY_LEVEL", "JUNIOR", "MID_LEVEL", "SENIOR", "LEAD", "PRINCIPAL", "EXECUTIVE"]
    
    # Map common variations to valid values to increase acceptance rate
    experience_mapping = {
        "ENTRY": "ENTRY_LEVEL",
        "JUNIOR": "JUNIOR", 
        "MID": "MID_LEVEL",
    ...
    if experience_level in experience_mapping:
        # Update the job_data with the mapped value
        job_data["experienceLevel"] = experience_mapping[experience_level]
    elif experience_level and experience_level not in valid_experience_levels:
        errors.append(f"experienceLevel must be one of the following values: {', '.join(valid_experience_levels)}")
    
    # Validate salary values - ensure they're integers as required by schema
    try:
        if "salaryMin" in job_data:
            salary_min = float(job_data.get("salaryMin", 0))
            job_data["salaryMin"] = int(salary_min)  # Convert to integer
    except (ValueError, TypeError):
        errors.append("salaryMin is not a valid decimal number")
        
    try:
        if "salaryMax" in job_data:
            salary_max = float(job_data.get("salaryMax", 0))
            job_data["salaryMax"] = int(salary_max)  # Convert to integer
    except (ValueError, TypeError):
        errors.append("salaryMax is not a valid decimal number")
