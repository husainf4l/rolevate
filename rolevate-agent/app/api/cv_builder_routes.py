"""
FastAPI endpoints for CV Builder AI Chatbot with streaming support
"""
import json
import uuid
...
from app.agent.cv_builder_workflow import CVBuilderWorkflow, CVBuilderState, IntentType
from app.models.user import User
from app.api.auth_routes import get_current_user_or_redirect
...
class ChatResponse(BaseModel):
    ...
    cv_progress: Dict[str, Any] = Field(default_factory=dict, description="CV building progress")
    suggestions: List[str] = Field(default_factory=list, description="Suggested next actions")


class GenerateCVRequest(BaseModel):
    """Generate CV request model."""
    conversation_id: str = Field(..., description="Conversation ID with CV data")
    template: Optional[str] = Field("modern", description="Template name")
    format: Optional[str] = Field("pdf", description="Output format")
