      ...
      <div className="bg-white/50 border border-[#d2d2d7] rounded-xl p-6 space-y-6">
        <div className="border-b border-[#d2d2d7] pb-4">
          ...
          {jobData.shortDescription && (
            <p className="text-[#6e6e73] bg-gray-50 p-3 rounded-lg text-sm">{jobData.shortDescription}</p>
          )}
        </div>
        ...
        {(jobData.aiCvAnalysisPrompt || jobData.aiFirstInterviewPrompt || jobData.aiSecondInterviewPrompt) && (
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4">
            ...
            <div className="space-y-3">
              {jobData.aiCvAnalysisPrompt && (
                <div>
                  <p className="text-sm font-medium text-blue-800 mb-1">CV Analysis Prompt:</p>
                  <p className="text-sm text-blue-700 bg-white/50 p-2 rounded">{jobData.aiCvAnalysisPrompt}</p>
                </div>
              )}
              {jobData.aiFirstInterviewPrompt && (
                <div>
                  <p className="text-sm font-medium text-blue-800 mb-1">First Interview Prompt:</p>
                  <p className="text-sm text-blue-700 bg-white/50 p-2 rounded">{jobData.aiFirstInterviewPrompt}</p>
                </div>
              )}
              {jobData.aiSecondInterviewPrompt && (
                <div>
                  <p className="text-sm font-medium text-blue-800 mb-1">Second Interview Prompt:</p>
                  <p className="text-sm text-blue-700 bg-white/50 p-2 rounded">{jobData.aiSecondInterviewPrompt}</p>
                </div>
              )}
            </div>
          </div>
