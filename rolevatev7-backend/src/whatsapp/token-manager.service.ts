...
export class TokenManagerService {
    ...
    async getAccessToken(): Promise<string> {
        ...
        if (cached && cached.expiresAt > new Date(Date.now() + 5 * TIME_CONSTANTS.MINUTE)) {
            return cached.token;
        }
    ...
    private async fetchAccessToken(): Promise<string> {
        try {
            ...
            if (staticToken) {
                ...
                this.cacheToken('whatsapp_access_token', staticToken, TIME_CONSTANTS.WHATSAPP_TOKEN_DURATION);
                return staticToken;
            }
            ...
            if (envSystemUserToken) {
                ...
                this.cacheToken('whatsapp_access_token', envSystemUserToken, TIME_CONSTANTS.WHATSAPP_TOKEN_DURATION);
            ...
            if (!appId || !appSecret || !systemUserId) {
                throw new BadRequestException('Missing Facebook app credentials. Please set FACEBOOK_APP_ID, FACEBOOK_APP_SECRET, and FACEBOOK_SYSTEM_USER_ID in your environment variables.');
            }
            ...
            this.cacheToken('whatsapp_access_token', systemUserToken, TIME_CONSTANTS.WHATSAPP_TOKEN_DURATION);
            ...
            throw new BadRequestException('Unable to obtain WhatsApp access token');
        }
    }
    ...
    private async getAppAccessToken(appId: string, appSecret: string): Promise<string> {
        ...
        if (!response.ok) {
            throw new BadRequestException(`Failed to get app access token: ${response.statusText}`);
        }
    ...
    private async getSystemUserToken(systemUserId: string, appAccessToken: string): Promise<string> {
        ...
        if (!response.ok) {
            throw new BadRequestException(`Failed to get system user token: ${response.statusText}`);
        }
        const data = await response.json();
        return data.access_token;
    }
